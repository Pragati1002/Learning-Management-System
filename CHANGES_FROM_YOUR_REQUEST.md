...
```
⚠️ `npm run seed` **deletes all existing users, courses, quizzes, mock tests, interviews, jobs, placements and assignments** in your database and replaces them with the updated set (including fresh admin/student accounts using the credentials configured in `server/.env`). If you have real student data in that database already, back it up first — otherwise this is exactly what you need to run to actually see the new content.
