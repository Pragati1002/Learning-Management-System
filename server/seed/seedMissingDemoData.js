// SAFE demo-data restore for RSR LMS.
// IMPORTANT: This script NEVER deletes users, courses, progress, quizzes, jobs,
// interviews, placements, mock tests, or assignments. It only inserts records
// that are missing. Run this when you need to restore demo content after a
// collection was accidentally emptied.
...
async function run() {
  ...
  // Make sure the MongoDB connection is fully ready before running queries.
  await mongoose.connection.asPromise();

  console.log('Starting SAFE demo-data restore. No existing records will be deleted.');
  ...
  for (const c of SEED_COURSES) {
    ...
    if (existing) {
      courseIdMap[c.mockId] = existing._id;
      continue;
    }
  ...
  // Build mappings for every seed course, including ones already in MongoDB.
  for (const c of SEED_COURSES) {
    if (!courseIdMap[c.mockId]) {
      const existing = await Course.findOne({ title: c.title });
      if (existing) courseIdMap[c.mockId] = existing._id;
    }
  }

  let addedQuizzes = 0;
  for (const q of SEED_QUIZZES) {
    const courseId = courseIdMap[q.mockCourseId];
    if (!courseId) continue;
    const exists = await Quiz.findOne({ title: q.title, courseId });
    if (exists) continue;
  ...
  for (const a of assignmentDefs) {
    ...
    if (!courseId) continue;
    const exists = await Assignment.findOne({ title: a.title, courseId });
    if (exists) continue;
    await Assignment.create({ courseId, title: a.title, description: a.description, dueDate: a.dueDate });
    addedAssignments++;
  }

  let addedMockTests = 0;
  for (const item of mockTests) {
    const exists = await MockTest.findOne({ title: item.title });
    if (exists) continue;
    await MockTest.create(item);
    addedMockTests++;
  }

  let addedInterviews = 0;
  for (const item of interviewTracks) {
    const exists = await InterviewTrack.findOne({ role: item.role });
    if (exists) continue;
    await InterviewTrack.create(item);
    addedInterviews++;
  }

  let addedJobs = 0;
  for (const item of jobs) {
    const exists = await Job.findOne({ title: item.title, company: item.company });
    if (exists) continue;
  ...
  for (const item of placements) {
    const exists = await Placement.findOne({ company: item.company, role: item.role });
    if (exists) continue;
    await Placement.create(item);
    addedPlacements++;
  }
  ...
  console.log('Existing user accounts and progress were NOT deleted or modified.');
