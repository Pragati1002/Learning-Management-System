...
function getBlueprint(course) {
  ...
  // IMPORTANT: course TITLE is the source of truth. Do not use the free-form
  // description to choose a blueprint. Descriptions often contain broad words
  // such as security, database, AI, web, etc. and caused unrelated courses to
  // receive the wrong content in earlier repairs.
  //
  // Exact/high-confidence title rules are checked first.
  const titleRules = [
    ['git', 'git'],
    ['github', 'git'],
    ...
    ['spring boot', 'java'],
    ['java spring', 'java'],
    ['rag applications with vector databases', 'llm'],
    ['rag application', 'llm'],
    ['react native', 'react'],
    ['next.js', 'react'],
    ['next js', 'react'],
  ...
  for (const [needle, key] of titleRules) {
    if (!title.includes(needle)) continue;
    const bp = BLUEPRINTS.find(item => item.key === key);
    if (bp) return bp;
  }

  // Then use the existing title-based BLUEPRINTS only. Never fall back to
  // description keyword matching.
  for (const bp of BLUEPRINTS) {
    if (bp.match.some(k => title === k || title.includes(k))) return bp;
  }

  // Category is a safer secondary signal than description text.
  const categoryRules = [
    ['developer tools', ['git']],
    ['version control', ['git']],
    ['cyber', ['cyber']],
    ['security', ['cyber']],
    ['programming', ['java', 'python', 'cpp', 'javascript']],
    ['python', ['python']],
  ...
  for (const [needle, keys] of categoryRules) {
    if (!category.includes(needle)) continue;
  ...
  // Finally, use a genuinely course-specific blueprint rather than borrowing
  // another subject's content.
...
async function repairCourse(course) {
  ...
  // The demo app intentionally exposes ONE practice quiz per course.
  // Reuse one existing quiz when possible, then remove the older quizzes for
  // this course. The quiz questions are based on the first repaired module.
  const quizModuleTitle = targetModules[0] || `${course.title} Practice`;
  const quizQuestionsForCourse = quizQuestions(quizModuleTitle).map(q => ({
    ...q,
    question: q.question.replace(quizModuleTitle, `${quizModuleTitle} in ${course.title}`)
  }));

  let quiz = await Quiz.findOne({ courseId: course._id }).sort({ createdAt: 1 }).exec();

  if (quiz) {
    quiz.title = `${course.title} - Practice Quiz`;
    quiz.courseTitle = course.title;
    quiz.durationMinutes = 10;
    quiz.passingScore = 60;
    quiz.status = 'published';
    quiz.questions = quizQuestionsForCourse;
    await quiz.save();
    ...
    console.log(`  Practice quiz cleanup: kept 1, removed ${deleted.deletedCount} old quiz(es).`);
  } else {
    quiz = await Quiz.create({
      ...
      passingScore: 60,
      status: 'published',
      questions: quizQuestionsForCourse
    });
    console.log('  Practice quiz created: 1');
  }
