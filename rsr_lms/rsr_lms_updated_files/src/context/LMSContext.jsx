...
/*
 * Dynamic Live Class normalizer.
 * Live classes are stored in MongoDB and linked to real Course ObjectIds.
 */
const normalizeLiveClass = (liveClass) => ({
  ...liveClass,
  id: String(liveClass.id || liveClass._id),
  courseId: String(
    liveClass.courseId?.id ||
...
export const LMSProvider = ({ children }) => {
  ...
  const [interviewTracks, setInterviewTracks] = useState(INITIAL_INTERVIEW_TRACKS);
  ...
  const [fees, setFees] = useState(() =>
    JSON.parse(localStorage.getItem('lms_fees_v3') || '[]')
  );

  const [leads, setLeads] = useState(() =>
    JSON.parse(localStorage.getItem('lms_leads_v3') || '[]')
  );

  const [tickets, setTickets] = useState(() =>
    JSON.parse(localStorage.getItem('lms_tickets_v3') || '[]')
  );

  // IMPORTANT: Live Classes are now dynamic.
  const [liveClasses, setLiveClasses] = useState([]);

  const [activeTab, setActiveTab] = useState('dashboard');
  const [courseSearchQuery, setCourseSearchQuery] = useState('');
  const [darkMode, setDarkMode] = useState(() =>
    localStorage.getItem('lms_theme_v3') === 'dark'
  );
  ...
  const loadData = async (user = currentUser) => {
    ...
    const tasks = [
      // Courses are backed by MongoDB. Use the database when it has data;
      // keep the original catalog only as a safe fallback if the API is empty.
      request('/courses', { token: token() })
        .then(data => {
          const list = Array.isArray(data) ? data : [];
          setCourses(list.length ? list.map(normalizeCourse) : INITIAL_COURSES);
        }),

      // Preserve the original built-in content when a backend collection is empty.
      request('/quizzes', { token: token() })
        .then(data => {
          const list = Array.isArray(data) ? data : [];
          setQuizzes(list.length ? list.map(normalizeQuiz) : INITIAL_QUIZZES);
        }),

      request('/mock-tests', { token: token() })
        .then(data => {
          const list = Array.isArray(data) ? data : [];
          setMockTests(list.length ? list.map(normalizeMockTest) : INITIAL_MOCK_TESTS);
        }),

      request('/interviews', { token: token() })
        .then(data => {
          const list = Array.isArray(data) ? data : [];
          setInterviewTracks(list.length ? list.map(normalizeInterview) : INITIAL_INTERVIEW_TRACKS);
        }),

      request('/jobs', { token: token() })
        .then(data => {
          const list = Array.isArray(data) ? data : [];
          setJobs(list.length ? list.map(normalizeJob) : INITIAL_JOBS);
        }),

      request('/placements', { token: token() })
        .then(data => {
          const list = Array.isArray(data) ? data : [];
          setPlacements(list.length ? list.map(normalizePlacement) : []);
        }),

      request('/assignments', { token: token() })
        .then(data => {
          const list = Array.isArray(data) ? data : [];
          setAssignments(list.length ? list.map(normalizeAssignment) : []);
        }),

      request('/discussions', { token: token() })
        .then(data => {
          const list = Array.isArray(data) ? data : [];
          setDiscussions(list.length ? list.map(normalizeDiscussion) : INITIAL_DISCUSSIONS);
        }),

      request('/certificates/mine', { token: token() })
        .then(data => setCertificates((data || []).map(normalizeCertificate))),

      request('/jobs/applications/mine', { token: token() })
        .then(data => setJobApplications((data || []).map(normalizeApplication))),

      // DYNAMIC LIVE CLASSES
      request('/live-classes', { token: token() })
        .then(data => setLiveClasses((data || []).map(normalizeLiveClass)))
    ];

    if (user.role === 'admin' || user.role === 'trainer') {
      tasks.push(
        request('/batches', { token: token() })
          .then(data => setBatches((data || []).map(normalizeBatch)))
      );
    } else {
      setBatches([]);
    }
    ...
    if (failures.length) {
      console.warn(
        'Some LMS data could not be loaded:',
  ...
  useEffect(
    () => localStorage.setItem(
      'lms_platform_feedback_v3',
      JSON.stringify(platformFeedback)
    ),
    [platformFeedback]
  );
  ...
  const getCourseProgress = (
    ...
    const allLessonIds =
      course.modules?.flatMap(
        m => m.lessons?.map(l => String(l.id)) || []
      ) || [];

    if (!allLessonIds.length) return 0;

    const completed = new Set(
      (user.completedLessons || []).map(String)
    );
  ...
  const toggleLessonComplete = async (
    ...
    try {
      const data = await request(
        `/courses/${courseId}/lessons/${encodeURIComponent(lessonId)}/complete`,
        {
          method: 'POST',
          token: token()
        }
      ...
      if (data.certificateIssued) {
        ...
        setCertificates(prev => [
          cert,
          ...prev.filter(
            c => c.certificateId !== cert.certificateId
          )
        ]);

        showToast(
          `🎓 Course finished! Certificate ${cert.certificateId} issued!`
        );
      } else {
        const done =
          updated.completedLessons.includes(
            String(lessonId)
          );

        showToast(
          done
            ? 'Lesson finished! +25 Learning XP ⭐'
            : 'Marked lesson as uncompleted',
          done ? 'success' : 'info'
        );
      }
    } catch (err) {
  ...
  const issueCertificateForCourse = (
    courseId,
    student = currentUser
  ) => {
    ...
    return certificates.find(
      c =>
        c.courseId === String(courseId) &&
        c.studentId === String(student.id)
    ) || null;
  };
  ...
  const addCourse = async data => {
    try {
      ...
      showToast(
        `Course "${created.title}" created successfully!`
      );
  ...
  const addQuiz = async data => {
    try {
      ...
      showToast(
        `Assessment "${created.title}" created!`
      );
  ...
  const submitQuizResult = async (
    ...
    try {
      ...
      if (
        ...
        showToast(
          `Passed ${quiz?.title || 'quiz'}! Score: ${score}%`,
          'success'
        );
      } else {
        showToast(
          `Score: ${score}%. Minimum passing is ${quiz?.passingScore || 70}%.`,
          'error'
        );
      }
  ...
  const submitMockTest = async (
    ...
    try {
      ...
      showToast(
        `Mock test submitted! Score: ${result.percentage}%`,
        'success'
      );
  ...
  const addStudentToBatch = async (
    ...
    try {
      ...
      showToast(
        `Student "${studentName}" added to batch.`
      );
    } catch (err) {
      showToast(err.message, 'error');
    }
  ...
  const addLiveClass = async data => {
    try {
      ...
      showToast(
        `Live class "${created.title}" scheduled!`
      );
