...
export const LMSProvider = ({ children }) => {
  ...
  const loadData = async (user = currentUser) => {
    ...
    if (user.role === 'admin' || user.role === 'trainer') {
      ...
      if (user.role === 'admin') tasks.push(request('/auth/users', { token: token() }).then(data => setUsers((data || []).map(u => ({ ...u, id: String(u.id || u._id) })) )));
    } else {
      setBatches([]);
    }
  ...
  const startCourseEnrollment = (course) => {
    ...
    if (price > 0) {
      // Paid courses NEVER call the enrollment endpoint directly.
  ...
  const completeCoursePayment = async (courseId) => {
    ...
    try {
      const data = await request(`/courses/${courseId}/payment/confirm`, {
        method: 'POST',
        body: { paymentRef: `DEMO-${Date.now()}` },
        token: token()
      });
      const updated = normalizeUser({ ...currentUser, enrolledCourses: data.enrolledCourses, paidCoursePayments: data.paidCoursePayments });
      setCurrentUser(updated);
      localStorage.setItem(USER_KEY, JSON.stringify(updated));
      setSelectedCourseForPayment(null);
      const course = courses.find(c => c.id === String(courseId));
      setSelectedCourseForPlayer(course || null);
  ...
  const enrollCourse = async (courseId) => {
    ...
    if (Number(course?.price || 0) > 0) {
      // Defense-in-depth: even an internal caller cannot enroll a paid course without payment.
      setSelectedCourseForPayment(course);
      setActiveTab('course-payment');
      showToast('Payment is required before enrolling in this paid course.', 'error');
      return false;
    }
    try {
      const data = await request(`/courses/${courseId}/enroll`, { method: 'POST', token: token() });
      const updated = normalizeUser({ ...currentUser, enrolledCourses: data.enrolledCourses, paidCoursePayments: data.paidCoursePayments || currentUser.paidCoursePayments });
      setCurrentUser(updated);
  ...
  const addQuiz = async (data) => {
    try {
      const payload = {
        title: data.title, courseId: data.courseId || undefined, courseTitle: data.courseTitle,
        institution: data.institution || '', durationMinutes: Number(data.durationMinutes) || 15,
        passingScore: Number(data.passingScore) || 70, status: data.status || 'draft',
      ...
      setQuizzes(prev => [created, ...prev]); showToast(`Quiz "${created.title}" created.`); return created;
    } catch (err) { showToast(err.message, 'error'); return null; }
  };
  ...
  const updateQuiz = async (quizId, data) => {
    try {
      ...
      setQuizzes(prev => prev.map(q => q.id === String(quizId) ? updated : q)); showToast('Quiz updated successfully.'); return updated;
    } catch (err) { showToast(err.message, 'error'); return null; }
  };

  const assignQuiz = async (quizId, studentIds) => {
    try {
      const updated = normalizeQuiz(await request(`/quizzes/${quizId}/assign`, { method: 'POST', body: { studentIds }, token: token() }));
      setQuizzes(prev => prev.map(q => q.id === String(quizId) ? updated : q)); showToast('Quiz assignment saved.'); return updated;
    } catch (err) { showToast(err.message, 'error'); return null; }
  };
  ...
  const deleteQuiz = async (quizId) => {
    try { await request(`/quizzes/${quizId}`, { method: 'DELETE', token: token() }); setQuizzes(prev => prev.filter(q => q.id !== String(quizId))); showToast('Quiz deleted.'); }
    catch (err) { showToast(err.message, 'error'); }
  };

  const updateModuleAccess = async (courseId, moduleId, isReleased) => {
    try {
      const updated = normalizeCourse(await request(`/courses/${courseId}/module-access`, { method: 'PUT', body: { moduleId, isReleased }, token: token() }));
      setCourses(prev => prev.map(c => c.id === String(courseId) ? updated : c));
      showToast(`Module ${isReleased ? 'released' : 'locked'} successfully.`);
      return updated;
    } catch (err) { showToast(err.message, 'error'); return null; }
  };
