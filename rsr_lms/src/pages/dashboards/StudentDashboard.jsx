...
export const StudentDashboard = ({ onOpenCertificate }) => {
  ...
  const paidIds = (currentUser?.paidCoursePayments || []).map(p => String(p.course));
  const hasAccess = course => enrolledIds.includes(String(course.id)) && (Number(course.price || 0) <= 0 || paidIds.includes(String(course.id)));
  const enrolledCourses = courses.filter(hasAccess);
  const discoverCourses = courses.filter(c => !hasAccess(c));
