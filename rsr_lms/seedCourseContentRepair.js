...
const GENERIC = {
  ...
  modules: [
    ...
    'Projects, Assessment & Interview Readiness'
  ]
};
...
async function repairCourse(course) {
  ...
  const generatedModules = (course.modules || [])
    .filter(m => String(m.moduleId || '').startsWith('exp_'))
    .sort((a, b) => String(a.moduleId).localeCompare(String(b.moduleId), undefined, { numeric: true }));

  // If generated modules are missing, create them. Otherwise repair exactly
  // the generated modules and preserve their existing IDs.
  for (let moduleIndex = 0; moduleIndex < blueprint.modules.length; moduleIndex++) {
    const moduleTitle = blueprint.modules[moduleIndex];
    let module = generatedModules[moduleIndex];

    if (!module) {
      module = {
        moduleId: `exp_${course._id}_${moduleIndex + 1}`,
        title: moduleTitle,
        lessons: []
      };
    ...
    module.title = `Module ${moduleIndex + 1}: ${moduleTitle}`;

    const titles = lessonTitles(moduleTitle);
    const videoPool = VIDEO_POOLS[blueprint.key] || VIDEO_POOLS.generic;

    for (let lessonIndex = 0; lessonIndex < titles.length; lessonIndex++) {
      const existing = module.lessons?.[lessonIndex];
      const lessonId =
        existing?.lessonId ||
        `exp_${course._id}_${moduleIndex + 1}_${lessonIndex + 1}`;

      const lesson = {
        lessonId,
        title: `${lessonIndex + 1}. ${titles[lessonIndex]}`,
        duration: `${15 + lessonIndex * 7} min`,
        videoUrl: videoPool[lessonIndex % videoPool.length],
        notes:
          `${titles[lessonIndex]} focuses on the practical concepts needed for ` +
          `${course.title}. Read the lesson notes, watch the demonstration, ` +
          `try the hands-on task, and review the interview questions before ` +
      ...
      if (existing) {
        // Mutate the existing subdocument so its lessonId is preserved.
        ...
        existing.resources = lesson.resources;
      } else {
        module.lessons.push(lesson);
      }
    }

    // Remove extra generated lessons beyond the four we intentionally define.
    if (Array.isArray(module.lessons) && module.lessons.length > 4) {
      module.lessons = module.lessons.slice(0, 4);
    }
    ...
    if (!quiz) {
      // Also repair the old expansion quiz if one exists for this course/module.
      const oldRegex = new RegExp(
        `^${course.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}.*Module ${moduleIndex + 1} Practice`,
        'i'
      );
    ...
    if (quiz) {
      ...
      quiz.questions = quizQuestions(moduleTitle);
      await quiz.save();
    } else {
      await Quiz.create({
        title: quizTitle,
        ...
        durationMinutes: 10,
        passingScore: 60,
        questions: quizQuestions(moduleTitle)
      });
    }
  }
